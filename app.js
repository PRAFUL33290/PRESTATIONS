...
function renderClientSelect(list) {
  ...
  el.innerHTML = list
    .map((item, i) => {
      const active = item.id === selectedClientId ? " is-active" : "";
      return `
        <button type="button" class="client-select-btn${active}" data-select-id="${escapeAttr(item.id)}">
          <span class="client-select-index">#${String(i + 1).padStart(2, "0")}</span>
          <span class="client-select-name">${escapeHtml(item.client)}</span>
          <span class="client-select-title">${escapeHtml(item.title)}</span>
        </button>`;
    })
    .join("");
  ...
  el.querySelectorAll("[data-select-id]").forEach((btn) => {
    btn.addEventListener("click", () => {
      selectedClientId = btn.getAttribute("data-select-id");
      refresh();
    });
  });
}
...
function renderCards(list) {
  ...
  empty.hidden = true;
  renderClientSelect(list);

  if (!selectedClientId || !list.some((item) => item.id === selectedClientId)) {
    selectedClientId = list[0].id;
    renderClientSelect(list);
  }

  const selectedList = list.filter((item) => item.id === selectedClientId);
